...
def resolve_project_name(agent) -> "str | None":
    ...
    Bug-1 note (activation race / new-chat): when a user clicks 'new chat'
    from a project in the UI the A0 chat_create handler propagates project
    data via set_data() rather than calling activate_project() on the new
    context.  If set_data() propagation completes before monologue_start
    fires tier-1 resolves correctly; if not (async ordering race) this
    function returns None for that first monologue.  The monologue_end flush
    extension re-calls this function before ending the span and retroactively
    patches the trace tags and agent.project OTel attribute.  Subsequent
    monologues resolve correctly once activate_project() has been called by
    the api_message handler.
    ...
    # Tier 1: direct context data key (set by activate_project or chat_create copy)
    name = _projects.get_context_project_name(context)
    if name:
        return name

    # Tier 2: output_data['project'] (also set by activate_project alongside data)
    try:
        out = context.get_output_data(_projects.CONTEXT_DATA_KEY_PROJECT)
    ...
    try:
        ...
        while superior is not None:
            ...
            if sup_ctx is not None:
                name = _projects.get_context_project_name(sup_ctx)
                if name:
                    return name
                try:
                    out = sup_ctx.get_output_data(_projects.CONTEXT_DATA_KEY_PROJECT)
                    if isinstance(out, dict):
                        name = out.get("name") or ""
                        if name:
                            return name
